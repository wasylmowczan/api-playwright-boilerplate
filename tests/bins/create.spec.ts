...
test.describe('POST /v3/b', () => {
    ...
    test.describe('when creating a new bin', () => {
        ...
        test('create a bit with header X-Bin-Private set to false', async ({ request }) => {
            // Given
            const newBin = {
                sample: 'Hello World'
            };
    ...
    test.describe('when request body is empty', () => {
        test('return 401 error when X-Access-Key is not provided', async ({ request }) => {
            // Given
            const newBin = {
                sample: 'Hello World'
            };
            ...
            const responseBody = await response.json();
            expect(responseBody.message).toBe('You need to pass X-Master-Key or X-Access-Key in the header to create a bin');
        });
